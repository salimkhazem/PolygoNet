...
def main(args):
    ...
    model = DeepNetwork(num_classes=len(classes))
    # model = DeepNetworkReduced(num_classes=len(classes))
    model.to(device)
    criterion = utils.get_criterion(cfg)
    optimizer = utils.get_optimizer(cfg, model)
    # scheduler = ReduceLROnPlateau(optimizer, mode="max", patience=5, factor=0.01, verbose=True)
    summary_text = (
        f"Logdir : {logdir}\n"
        + "## Command \n"
        + " ".join(sys.argv)
    ...
    epoch_bar = tqdm(
        range(cfg["Training"]["Epochs"]), total=int(cfg["Training"]["Epochs"])
    )
        ...
        with tqdm(
            ...
            for batch in train_loader:
                ...
                if (
                    ...
                    all_true_labels.extend(valid_metrics["CM"].index.tolist())
                    ...
                    if valid_metrics["Accuracy"] > best_results:
                        ...
                        logging.info(
                            f"=== Best model saved with Accuracy score: {valid_metrics['Accuracy']} ===="
                        )
    ...
    cf_matrix = metrics.confusion_matrix(all_true_labels, all_predicted_labels)
    df_cm = pd.DataFrame(
